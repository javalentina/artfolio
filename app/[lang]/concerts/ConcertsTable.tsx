        ...
        {list.map(c => {
          ...
          return (
            <>
              <tr
                key={c.id}
                onClick={() => hasPhotos && setOpenId(isOpen ? null : c.id)}
                className={`border-b border-border transition-colors hover:bg-primary/[0.04] ${dim ? "opacity-60" : ""} ${hasPhotos ? "cursor-pointer" : "cursor-default"}`}
              >
                ...
                <td className="py-[18px] text-right w-28">
                  <div className="flex items-center justify-end gap-3">
                    {hasPhotos && (
                      <span className="text-[0.6rem] uppercase tracking-[0.15em] text-primary/50">
              ...
              {isOpen && hasPhotos && (
                <tr key={`${c.id}-gallery`}>
                  <td colSpan={4} className="pb-6 pt-2">
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                      {photos.map((src, i) => (
                        /* eslint-disable-next-line @next/next/no-img-element */
                        <img key={i} src={src} alt="" className="w-full h-auto rounded-lg object-cover" />
                      ))}
                    </div>
                  </td>
                </tr>
